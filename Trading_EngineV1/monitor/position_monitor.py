"""
Position monitoring system for real-time position tracking and risk assessment.
"""
import time
import threading
...
class PositionMonitor:
    ...
    def __init__(self, exchange_client: BinanceFuturesClient, alert_callback: Optional[Callable] = None, 
    ...
    def _compare_positions(self, current_positions: Dict[str, PositionMetrics]):
        ...
        for symbol, current_pos in current_positions.items():
            if symbol in self.last_positions:
                ...
                if abs(pnl_change) > 10:
                    self._create_alert(
                        symbol=symbol,
                        alert_type="pnl_change",
                        level=AlertLevel.INFO,
                        message=f"P&L changed by ${pnl_change:.2f}",
                        data={'old_pnl': last_pos.unrealized_pnl, 'new_pnl': current_pos.unrealized_pnl, 'change': pnl_change}
                    )
                ...
                if current_pos.quantity != last_pos.quantity:
                    self._create_alert(
                        ...
                        data={'old_quantity': last_pos.quantity, 'new_quantity': current_pos.quantity}
                    )
            else:
                # New position
                self._create_alert(
                    ...
                    data={'quantity': current_pos.quantity, 'entry_price': current_pos.entry_price, 'side': current_pos.side}
                )
        ...
        for symbol, pos in positions.items():
            if pos.pnl_percentage < self.risk_thresholds['max_pnl_loss_percent']:
                self._create_alert(
                    ...
                    data={'pnl_percentage': pos.pnl_percentage, 'threshold': self.risk_thresholds['max_pnl_loss_percent']}
                )
            ...
            if pos.position_value > self.risk_thresholds['max_position_value']:
                self._create_alert(
                    ...
                    data={'position_value': pos.position_value, 'threshold': self.risk_thresholds['max_position_value']}
                )
            ...
            if pos.leverage > self.risk_thresholds['max_leverage']:
                self._create_alert(
                    ...
                    data={'leverage': pos.leverage, 'threshold': self.risk_thresholds['max_leverage']}
                )
        ...
        if total_margin > 0:
            ...
            if margin_ratio > self.risk_thresholds['max_margin_ratio']:
                self._create_alert(
                    ...
                    data={'margin_ratio': margin_ratio, 'threshold': self.risk_thresholds['max_margin_ratio'], 'total_margin': total_margin, 'total_pnl': total_pnl}
                )
                
    def _create_alert(self, symbol: str, alert_type: str, level: AlertLevel, message: str, data: Dict):
